  ...
  getFirebase(dataRef, realtime) {
    ...
    Promise.all([
      fetch(`${dataRef}.json?shallow=true`)
        .then(resp => resp.json())
        .then(json => {
          withDelete = (json.withDelete ? json.withDelete : false)
          return ({ mode: MODE_OFFLINE, json: json.data })
        }),
      realtime
        ? fetch(`${dataRef}.json`, {
          signal
          ...
          .then(resp => resp.json())
          .then(json => ({ mode: MODE_REALTIME, json: json.data }))
          .catch(e => {
      ...
      .then(result => {
        const { json, mode, withDelete } = result;
